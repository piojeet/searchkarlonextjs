...
const LoanTimePeriod = ({
    ...
    const handleInputChange = (e) => {
        ...
        if (isYearMode) {
            if (inputValue >= minYear && inputValue <= maxYear) {
                setValue(inputValue);
                onChange(inputValue);
            }
        } else {
            if (inputValue >= minMonth && inputValue <= maxMonth) {
                setValue(inputValue);
                onChange(inputValue);
            }
        }
            ...
            <div className="text-sm flex items-center justify-between pb-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1 border-2 py-1 px-2 rounded-md">
                        <label className={`px-2 py-1 rounded ${isYearMode ? "bg-lightOrange text-white" : "bg-transparent"
                            }`}>
                            <input
                                type="radio"
                                name="mode"
